function [solutions, f_values, f2_values, c_values, a_values] = ABCTime(starter, Np, T, objective, lb, ub, d, limit)
...
%     P = random(lb, ub, Np, d, 'real');
    ...
    %f = fitness(P);
    for i = 1:Np
        [f(i), counter(i), actual(i)] = objective(P(i,:));
        f2(i) = fitness(f(i));
    end
    ...
    for t = 1:T
        ...
        for i = 1:Np
            ...
            [f_new, c_new, a_new] = objective(X_new);
        ...
        temp = [counter; c_values(t)];
        c_values(t+1) = temp(j_best);
        
        temp = [actual; a_values(t)];
        a_values(t+1) = temp(j_best);
        ...
        if val > limit
            ...
            [f(ind), counter(ind), actual(ind)] = objective(P(ind,:));
